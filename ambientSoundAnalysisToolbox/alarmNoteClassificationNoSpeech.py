...
from model_utils import plot_multi_pr, plot_multi_roc
...
mfccFeaturesFile = '/path/to/repo/OS_Edge_Compute_Data_Capture_RPi/ambientSoundAnalysisToolbox/data/features/mfccFeatures.mat'
mfccFeaturesDict = sio.loadmat(mfccFeaturesFile)
mfccFeatures = mfccFeaturesDict['mfccMat']
mfccVar = mfccFeaturesDict['mfccVar']

energyFeaturesFile = '/path/to/repo/OS_Edge_Compute_Data_Capture_RPi/ambientSoundAnalysisToolbox/data/features/energyFeatures.mat'
...
#X = np.transpose(np.concatenate((mfccFeatures, energyFeatures),0))
X = np.transpose(np.concatenate((mfccFeatures, energyFeatures, mfccVar, energyVar),0))
#featNos = np.linspace(0,19,20) # MSE
#X = X[:,featNos]
...
y_train_prob_meta_xg = np.zeros((X.shape[0], nClasses))
y_train_prob_meta_rf = np.zeros((X.shape[0], nClasses))
y_train_prob_meta_lr = np.zeros((X.shape[0], nClasses))
y_train_prob_meta_svc = np.zeros((X.shape[0], nClasses))
feat_wght_xg = np.zeros(X.shape[1])
feat_wght_rf = np.zeros(X.shape[1])
feat_wght_lr = np.zeros(X.shape[1])
feat_wght_svc = np.zeros(X.shape[1])

# for idx, (train_idx, val_idx) in enumerate(loo.split(X)):
for idx, (train_idx, val_idx) in enumerate(skf.split(X, Y)):
    ...
    X_train_splt = X[train_idx]
    y_train_splt = Y[train_idx]

    X_val_splt = X[val_idx]
